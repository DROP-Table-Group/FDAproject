...
def train_and_evaluate(
    ...
    for epoch in range(num_epochs):
        train_loss, _, _ = train_one_epoch(model, train_loader, optimizer, criterion, device)
...
class BayesianOptimizationRunner:
    def __init__(
        ...
        self.dim_names = ['learning_rate', 'weight_decay', 'dropout', 'fc1_dim', 'batch_size']
    ...
    def objective(self, params):
        ...
        print(f"\n尝试超参数组合:")
        print(f"  learning_rate={learning_rate:.6f}, weight_decay={weight_decay:.6f}, "
    ...
    def run(self, save_dir: str = './bayesian_opt_results'):
        """
        运行贝叶斯优化
        """
        print(f"\n{'='*60}")
    ...
    def _save_results(self, result, save_dir: Path):
        ...
        best_params = {
            name: (int(result.x[i]) if isinstance(result.x[i], (np.integer, np.int64)) else # pyright: ignore[reportArgumentType]
                   float(result.x[i]) if isinstance(result.x[i], (np.floating, np.float64)) else
                   result.x[i])
            for i, name in enumerate(self.dim_names)
        }
        ...
        history_df.to_csv(save_dir / f'optimization_history_{timestamp}.csv', index=False)

        # 保存收敛曲线图数据
        convergence_data = {
            'iteration': list(range(1, len(self.history) + 1)),
            'best_test_loss': [min([h['test_loss'] for h in self.history[:i+1]])
...
def main():
    ...
    parser.add_argument('--data-path', type=str, default='./data/btcusd_1-min_data.csv',
                        help='输入数据路径')
